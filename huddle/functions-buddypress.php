...
add_action( 'nxt_ajax_members_filter', 'bp_dtheme_object_template_loader' );
add_action( 'nxt_ajax_groups_filter',  'bp_dtheme_object_template_loader' );
add_action( 'nxt_ajax_blogs_filter',   'bp_dtheme_object_template_loader' );
add_action( 'nxt_ajax_forums_filter',  'bp_dtheme_object_template_loader' );
...
add_action( 'nxt_ajax_activity_widget_filter', 'bp_dtheme_activity_template_loader' );
add_action( 'nxt_ajax_activity_get_older_updates', 'bp_dtheme_activity_template_loader' );
...
function bp_dtheme_post_update() {
	...
	// Check the nonce
	check_admin_referer( 'post_update', '_nxtnonce_post_update' );
...
add_action( 'nxt_ajax_post_update', 'bp_dtheme_post_update' );
...
add_action( 'nxt_ajax_new_activity_comment', 'bp_dtheme_new_activity_comment' );
...
add_action( 'nxt_ajax_delete_activity', 'bp_dtheme_delete_activity' );
...
add_action( 'nxt_ajax_delete_activity_comment', 'bp_dtheme_delete_activity_comment' );
...
add_action( 'nxt_ajax_activity_mark_fav', 'bp_dtheme_mark_activity_favorite' );
...
add_action( 'nxt_ajax_activity_mark_unfav', 'bp_dtheme_unmark_activity_favorite' );
...
add_action( 'nxt_ajax_get_single_activity_content', 'bp_dtheme_get_single_activity_content' );
...
function bp_dtheme_ajax_invite_user() {
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="button remove" href="' . nxt_nonce_url( $bp->loggedin_user->domain . bp_get_groups_slug() . '/' . $_POST['group_id'] . '/invites/remove/' . $user->id, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'huddle' ) . '</a>
			  </div>';
		echo '</li>';
...
add_action( 'nxt_ajax_groups_invite_user', 'bp_dtheme_ajax_invite_user' );
...
function bp_dtheme_ajax_addremove_friend() {
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
		...
		if ( !friends_remove_friend( $bp->loggedin_user->id, $_POST['fid'] ) ) {
			...
			echo '<a id="friend-' . $_POST['fid'] . '" class="add" rel="add" title="' . __( 'Add Friend', 'huddle' ) . '" href="' . nxt_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/add-friend/' . $_POST['fid'], 'friends_add_friend' ) . '">' . __( 'Add Friend', 'huddle' ) . '</a>';
		}
...
add_action( 'nxt_ajax_addremove_friend', 'bp_dtheme_ajax_addremove_friend' );
...
add_action( 'nxt_ajax_accept_friendship', 'bp_dtheme_ajax_accept_friendship' );
...
add_action( 'nxt_ajax_reject_friendship', 'bp_dtheme_ajax_reject_friendship' );
	...
	if ( !groups_is_user_member( $bp->loggedin_user->id, $group->id ) ) {
		...
		if ( 'public' == $group->status ) {
			...
			if ( !groups_join_group( $group->id ) ) {
				...
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'huddle' ) . '" href="' . nxt_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'huddle' ) . '</a>';
			}
		...
		if ( !groups_leave_group( $group->id ) ) {
			...
			if ( 'public' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group" rel="join" title="' . __( 'Join Group', 'huddle' ) . '" href="' . nxt_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'huddle' ) . '</a>';
			} else if ( 'private' == $group->status ) {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'huddle' ) . '" href="' . nxt_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'huddle' ) . '</a>';
			}
		}
	}
}
add_action( 'nxt_ajax_joinleave_group', 'bp_dtheme_ajax_joinleave_group' );
...
add_action( 'nxt_ajax_messages_close_notice', 'bp_dtheme_ajax_close_notice' );
...
add_action( 'nxt_ajax_messages_send_reply', 'bp_dtheme_ajax_messages_send_reply' );
...
add_action( 'nxt_ajax_messages_markunread', 'bp_dtheme_ajax_message_markunread' );
...
add_action( 'nxt_ajax_messages_markread', 'bp_dtheme_ajax_message_markread' );
...
add_action( 'nxt_ajax_messages_delete', 'bp_dtheme_ajax_messages_delete' );
...
add_action( 'nxt_ajax_messages_autocomplete_results', 'bp_dtheme_ajax_messages_autocomplete_results' );

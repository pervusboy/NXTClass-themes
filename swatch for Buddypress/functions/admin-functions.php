...
function woo_image($args) {
		...
		if ( !empty($attachments) ) {
			...
			foreach ( $attachments as $att_id => $attachment ) {
				...
				if ( get_option( 'woo_post_image_support' ) == "true" AND get_option( 'woo_pis_resize') == "true" ) {
					...
					$src = nxt_get_attachment_image_src($att_id, $size, true);
					$custom_field = $src[0];
	...
	if(!empty($img_link) ){
		...
		if( $link == 'img' ) {  // Output the image without anchors
			...
			$src = nxt_get_attachment_image_src($thumb_id, 'large', true);
			$custom_field = $src[0];
			$output .= $custom_field;
	...
	elseif ( get_option( 'woo_resize') == 'true' && empty($vt_image['url']) ) {
		...
		foreach($src_arr as $key => $custom_field){
			...
			if ($meta) {
				...
				if ($alt == '' AND get_post_meta($thumb_id, '_nxt_attachment_image_alt', true) )
					$alt = get_post_meta($thumb_id, '_nxt_attachment_image_alt', true);
				else
	...
	// No dynamic resizing
		...
		foreach($src_arr as $key => $custom_field){
			...
			if ($meta) {
				...
				if ($alt == '') $alt = get_post_meta($thumb_id, '_nxt_attachment_image_alt', true);
				$title = 'title="'. get_the_title($quick_id) .'"';
			}
			...
			if ( $link == 'img' ) {  // Just output the image
				...
				if ( $vt_image['url'] || $is_auto_image ) {
					$src = nxt_get_attachment_image_src($thumb_id, 'full', true);
					$custom_field = $src[0];
				}
				$output .= $custom_field;
...
/*-----------------------------------------------------------------------------------*/
/* vt_resize - Resize images dynamically using nxt built in functions
/*-----------------------------------------------------------------------------------*/
/*
 * Resize images dynamically using nxt built in functions
 * Victor Teixeira
 *
 * php 5.2+
 *
...
function woo_active_template($filename = null){
	...
	if(isset($filename)){
		...
		global $nxtdb;
		$query = "SELECT *,count(*) AS used FROM $nxtdb->postmeta WHERE meta_key = '_nxt_page_template' AND meta_value = '$filename' GROUP BY meta_value";
		$results = $nxtdb->get_row($nxtdb->prepare($query),'ARRAY_A' ); // Select thrid coloumn accross
...
function woothemes_framework_update_page() {
	...
	delete_transient( 'wooframework_version_data' );
        ...
        $to = ABSPATH . 'nxt-content/themes/' . get_option( 'template' ) . '/functions/';
...
function woo_get_fw_version( $url = '', $check_if_critical = false ) {
	...
	if( ! is_nxt_error( $temp_file_addr ) && $file_contents = file( $temp_file_addr ) ) {
...
function woo_title(){
	...
	&&
		( $use_third_party_data == true ) ) { nxt_title(); return; }
	...
	if($use_nxt_title == 'true'){
		...
		if( get_option( 'seo_woo_nxt_custom_field_title') != 'true' && is_singular() ) {
			...
			switch ( $layout ) {
				case 'a': $output = nxt_title($sep,false,true) . get_bloginfo( 'name' );
				break;
				case 'b': $output = nxt_title( '',false,false);
				break;
				case 'c': $output = get_bloginfo( 'name') . nxt_title($sep,false,false);
				break;
				case 'd': $output = nxt_title($sep,false,true) . get_bloginfo( 'description' );
				break;
				case 'e': $output = get_bloginfo( 'name') . $sep . nxt_title($sep,false,true) . get_bloginfo( 'description' );
				break;
				...
				break;
				case 'nxtseo':  $output = $nxtseo_title; // NXTClass SEO Title
				break;
			}
			if(is_paged()){
				$paged_var = get_query_var( 'paged' );
...
function woo_meta(){
		...
		// Under SETTIGNS > PRIVACY in the NXTClass backend
		...
		} elseif( ! empty( $nxtseo_desc ) AND $use_third_party_data) {
			$singular_desc_option = 'nxtseo';
		}
		...
		elseif(is_singular()){
			...
			switch($singular_desc_option){
				...
				break;
				case 'nxtseo':  $description = $nxtseo_desc; // NXTClass SEO Description
				break;
		...
		} elseif( ! empty( $nxtseo_keywords ) AND $use_third_party_data) {
			$singular_key_option = 'nxtseo';
		}
		...
		elseif(is_singular()){
			...
			switch($singular_key_option){
				...
				break;
				case 'nxtseo':  $keywords = $nxtseo_keywords; // Headspace Title
				break;
				}
		}
...
if ( ! function_exists( 'woo_pagination' ) ) {
	...
	function woo_pagination( $args = array(), $query = '' ) {
		...
		$max_num_pages = intval( $nxt_query->max_num_pages );
		...
		/* If we're in BuddyPress, or the user has selected to do so, use the default "unpretty" URL structure. */
			if ( class_exists( 'BP_Core_User' ) || $defaults['use_search_permastruct'] == false ) {
				...
				$search_permastruct = $nxt_rewrite->get_search_permastruct();
		...
		if( $nxt_rewrite->using_permalinks() && $raw_querystring )
...
function woo_breadcrumbs( $args = array() ) {
	...
	elseif ( is_singular() ) {
		...
		$post = $nxt_query->get_queried_object();
		$post_id = absint( $nxt_query->get_queried_object_id() );
		...
		if ( 'page' !== $post_type && 'post' !== $post_type ) {
			...
			if ( 'post' == $post_type || 'attachment' == $post_type || ( $post_type_object->rewrite['with_front'] && $nxt_rewrite->front ) )
				$path .= trailingslashit( $nxt_rewrite->front );
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			elseif ( is_tag() )
				...
				if ( $taxonomy->rewrite['with_front'] && $nxt_rewrite->front )
					$path = trailingslashit( $nxt_rewrite->front );
		...
		elseif ( function_exists( 'is_post_type_archive' ) && is_post_type_archive() ) {
			...
			/* If $front has been set, add it to the $path. */
			if ( $post_type_object->rewrite['with_front'] && $nxt_rewrite->front )
				$path .= trailingslashit( $nxt_rewrite->front );
		...
		elseif ( is_author() ) {
			...
			if ( !empty( $nxt_rewrite->front ) )
				$path .= trailingslashit( $nxt_rewrite->front );

			/* If an $author_base exists, add it to $path. */
			if ( !empty( $nxt_rewrite->author_base ) )
				$path .= $nxt_rewrite->author_base;

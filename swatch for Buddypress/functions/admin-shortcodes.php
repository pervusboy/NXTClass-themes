...
function _dep_woo_shortcode_related_posts( $atts ) {
	...
	global $nxtdb, $post, $table_prefix;
...
<ul class="woo-sc-related-posts">';
		...
		$q = "
			...
			FROM $nxtdb->term_taxonomy AS tt, $nxtdb->term_relationships AS tr, $nxtdb->posts AS p
		...
		if ( $related ) {
			...
			foreach($related as $r) {
				...
				$retval .= '
	<li>'.$image_out.'<a class="related-title" title="'.nxttexturize($r->post_title).'" href="'.get_permalink($r->ID).'"><span>'.nxttexturize($r->post_title).'</span></a></li>
';
			}
		} else {
			$retval .= '

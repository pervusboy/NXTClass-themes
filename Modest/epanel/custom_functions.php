...
if (function_exists('create_initial_post_types')) create_initial_post_types(); //fix for nxt 3.0 beta 1
...
if ( ! function_exists( 'get_thumbnail' ) ){
	function get_thumbnail($width=100, $height=100, $class='', $alttext='', $titletext='', $fullpath=false, $custom_field='', $post='')
	{
		...
		if ( has_post_thumbnail( $post->ID ) && !( '' != $custom_field && get_post_meta( $post->ID, $custom_field, true ) ) ) {
			...
			$et_fullpath =  nxt_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
	...
	function print_thumbnail($thumbnail = '', $use_timthumb = true, $alttext = '', $width = 100, $height = 100, $class = '', $echoout = true, $forstyle = false, $resize = true, $post='') {
		...
		if ($use_timthumb === true) {
			...
			if ( $allow_new_thumb_method && $thumbnail <> '' ){
				...
				if ( is_nxt_error( $new_method_thumb ) ) $new_method_thumb = '';
			}
		}
	...
	function et_multisite_thumbnail($thumbnail='') {
		...
		if (isset($blog_id) && $blog_id > 0) {
			...
			if (isset($imagePath[1])) {
				$thumbnail = apply_filters( 'et_multisite_thumbs_absolute_path', get_home_url( 1 ) . '/nxt-content/' ); // retrieves nxt-content url for a main site ( with blog_id = 1 )
...
if ( ! function_exists( 'et_is_portrait' ) ){
	function et_is_portrait($imageurl, $post='', $ignore_cfields = false){
		...
		if ( empty($et_thumb_size) ) {
			$et_thumb_size = @getimagesize( str_replace( nxt_CONTENT_URL, nxt_CONTENT_DIR, $imageurl ) );
...
add_action('nxt_head','head_addons',7);
...
add_action('nxt_head','integration_head',12);
...
add_action('nxt_footer','integration_body',12);
...
if ( ! function_exists( 'get_pageId' ) ){
	function get_pageId( $page_name )
	{
		...
		#if ( $page_name_id == '' ) $page_name_id = $nxtdb->get_var("SELECT ID FROM $nxtdb->posts WHERE post_title LIKE '%".trim($page_name)."%' AND post_type = 'page'");
		...
		if (is_single() || is_page()) { 
			...
			$exists3 = get_post_meta($postid, ''.$key.'', true);
					if (get_option($shortname.'_seo_single_title') == 'on' && $exists3 !== '' ) echo $exists3; 
						...
						if (get_option($shortname.'_seo_single_type') == 'BlogName | Post title') echo get_bloginfo('name').get_option($shortname.'_seo_single_separate').nxt_title('',false,''); 
						if ( get_option($shortname.'_seo_single_type') == 'Post title | BlogName') echo nxt_title('',false,'').get_option($shortname.'_seo_single_separate').get_bloginfo('name');
						if ( get_option($shortname.'_seo_single_type') == 'Post title only') echo nxt_title('',false,'');
					}
						
		}
		#if the title is being displayed on index pages (categories/archives/search results)
		if (is_category() || is_archive() || is_search()) { 
			if (get_option($shortname.'_seo_index_type') == 'BlogName | Category name') echo get_bloginfo('name').get_option($shortname.'_seo_index_separate').nxt_title('',false,''); 
			if ( get_option($shortname.'_seo_index_type') == 'Category name | BlogName') echo nxt_title('',false,'').get_option($shortname.'_seo_index_separate').get_bloginfo('name');
			if ( get_option($shortname.'_seo_index_type') == 'Category name only') echo nxt_title('',false,'');
			 }	  
	}
}
...
if ( ! function_exists( 'elegant_description' ) ){
	function elegant_description() {
		...
		global $nxt_query; 
		if (isset($nxt_query->post->ID)) $postid = $nxt_query->post->ID; 
		...
		if (is_archive() && get_option($shortname.'_seo_index_description') == 'on') echo '<meta name="description" content="Currently viewing archives from'. nxt_title('',false,'') .'" />';
		if (is_search() && get_option($shortname.'_seo_index_description') == 'on') echo '<meta name="description" content="'. nxt_title('',false,'') .'" />';
	}
}
...
add_action('nxt_head','add_favicon');
...
function et_create_images_temp_folder(){
	...
	if ( ! nxt_mkdir_p( $destination_dir ) ) update_option( 'et_images_temp_folder', '' );
	else { 
...
if ( ! function_exists( 'et_resize_image' ) ){
	function et_resize_image( $thumb, $new_width, $new_height, $crop ){
		...
		if ( !$size ) return new nxt_Error('invalid_image_path', __('Image doesn\'t exist'), $thumb);
		...
		if ( $orig_width > $new_width || $orig_height > $new_height ){
			...
			#returns resized image path or nxt_Error ( if something went wrong during resizing )
			return $result;
		}

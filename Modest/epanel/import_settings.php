...
function import_epanel_javascript( $hook_suffix ) {
	if ( 'admin.php' == $hook_suffix && isset( $_GET['import'] ) && isset( $_GET['step'] ) && 'nxtclass' == $_GET['import'] && '1' == $_GET['step'] )
...
function importend(){
	global $nxtdb, $shortname;
	
	#make custom fields image paths point to sampledata/sample_images folder
	$sample_images_postmeta = $nxtdb->get_results("SELECT meta_id, meta_value FROM $nxtdb->postmeta WHERE meta_value REGEXP 'http://et_sample_images.com'");
	if ( $sample_images_postmeta ) {
		foreach ( $sample_images_postmeta as $postmeta ){
			...
			if ( is_multisite() ){
				...
				$template_dir = $main_siteurl . '/nxt-content/themes/' . get_template();
			}
			...
			$nxtdb->update( $nxtdb->postmeta, array( 'meta_value' => $local_image ), array( 'meta_id' => $postmeta->meta_id ), array( '%s' ) );
		}
	}

...
/* Adds jquery script */
add_action('nxt_print_scripts', 'et_jquery_script',8);
function et_jquery_script(){
	if ( function_exists('esc_attr') ) nxt_enqueue_script('jquery'); 
	else {
...
if ( ! function_exists( 'et_epanel_admin_js' ) ){
	function et_epanel_admin_js(){
		...
		nxt_localize_script( 'epanel_functions_init', 'ePanelSettings', array(
				'clearpath' => get_template_directory_uri() . '/epanel/images/empty.png',
				'epanel_nonce' => nxt_create_nonce('epanel_nonce')
		));
...
/* Save/Reset actions | Adds theme options to nxt-Admin menu */
		...
		<?php 
			...
			<div style="position: relative;">
				<div class="defaults-hover">
					...
					<div class="clearfix"></div>
					<form method="post">
						<?php nxt_nonce_field('et-nojs-reset_epanel'); ?>
...
add_action('nxt_ajax_save_epanel', 'et_epanel_save_callback');

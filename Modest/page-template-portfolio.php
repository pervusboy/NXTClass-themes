		...
		<div class="entry clearfix post">	
			...
			<?php nxt_link_pages(array('before' => '<p><strong>'.esc_html__('Pages','Modest').':</strong> ', 'after' => '</p>', 'next_or_number' => 'number')); ?>
						...
						<div id="et_pt_portfolio_gallery" class="clearfix<?php echo $et_ptemplate_portfolio_class; ?>">
							...
							?>
							<?php query_posts("shonxtosts=$et_ptemplate_gallery_perpage&paged=" . $et_paged . $gallery_query); ?>
							<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
								...
								if ( has_post_thumbnail( $post->ID ) && $et_ptemplate_detect_portrait ) {
									$nxtclass_thumbnail = get_post( get_post_thumbnail_id($post->ID) );
									$nxtclass_thumbnail_url = $nxtclass_thumbnail->guid;
									
									if ( et_is_portrait($nxtclass_thumbnail_url) )	$height = $et_portrait_height;
								}
										...
										<div class="et_pt_portfolio_item">
											...
											<div class="et_pt_portfolio_entry<?php if ( $height == $et_portrait_height ) echo ' et_portrait_layout'; ?>">
												<div class="et_pt_portfolio_image<?php if ($et_videolink <> '') echo ' et_video'; ?>">
													...
													<a class="et_portfolio_zoom_icon <?php if ($et_videolink <> '') echo 'et_video_lightbox'; else echo('fancybox'); ?>" title="<?php the_title(); ?>"<?php if ($et_videolink == '') echo ' rel="portfolio"'; ?> href="<?php if ($et_videolink <> '') echo $et_videolink; else echo($thumbnail['fullpath']); ?>"><?php esc_html_e('Zoom in','Modest'); ?></a>
							...
							endwhile; ?>
								...
								<div class="page-nav clearfix">
									<?php if (function_exists('nxt_pagenavi')) { nxt_pagenavi(); }
									else { ?>

...
class ICE_Scheme_Enqueue extends ICE_Base
{
	...
	public function __construct( ICE_Scheme $scheme )
	{
		...
		if ( $this->script_domain instanceof ICE_Init_Directive ) {
			add_action( 'nxt_print_scripts', array( $this, 'handle_script_domain' ) );
	...
	public function handle_script_internal()
	{
		...
		if ( is_admin() ) {
			...
			if ( nxt_script_is( '@:dynamic-admin', 'registered' ) ) {
				nxt_enqueue_script( '@:dynamic-admin' );
			}
		} else {
			// enqueue public scripts
			if ( nxt_script_is( '@:dynamic', 'registered' ) ) {
				nxt_enqueue_script( '@:dynamic' );
			}
		}
	}

...
if ( current_theme_supports( 'infinity-pagination' ) ) 
{
	...
	function infinity_base_paginate() {
	   ...
	   $nxt_query->query_vars['paged'] > 1 ? $current = $nxt_query->query_vars['paged'] : $current = 1;
	
	   $pagination = array(
	       'base' => @add_query_arg('page','%#%'),
	       'format' => '',
	       'total' => $nxt_query->max_num_pages,
	       'current' => $current,
      	   'show_all' => false,
		   'end_size' => 3,
		   'mid_size' => 5,
	   ...
	   if ( !empty( $nxt_query->query_vars['s'] ) ) {
...
/**
 * Add Breadcrumb functionality for NXTClass SEO
 *
 * @package Infinity
 * @subpackage base
...
function infinity_cleaner_caption( $output, $attr, $content ) 
{
	...
	$output .= '<figcaption class="nxt-caption">' . $attr['caption'] . '</figcaption>';
...
/**
 * Clean the output of attributes of images in editor. Courtesy of SitePoint. http://www.sitepoint.com/nxtclass-change-img-tag-html/
 *
 * @package Infinity
 * @subpackage base
 */
